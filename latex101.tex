...
\begin{frame}
  ...
  \item   Or Lots of editors/GUIs available.

  \item   I suggest trying texstudio.  It handles all the compilation steps
  for you and provides easy way of forward/inverse searching (Ctrl+
  left mouse button).

\item \url{http://www.lyx.org} = latex engine + WYSIWG interface.
\end{itemize}
...
\begin{frame}
  ...
  Many \latex guides describe how you can create .dvi files and .ps
  (postscript) files.  
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
  \item \latex normally is in tex mode.  You must switch to math mode
...
getting things ``just right'', e.g. the differential operator ``d'' as in
...
\begin{verbatim}
We used the R programming environment (Ihaka and Gentelman, 
...
\begin{frame}
  ...
  how often to re-reun \latex to resolve references.
...
Acknowledgements: thanks to Robert Stojnic and Markus Kuhn, who wrote
